...
type
  ...
  // --- result value ----
  TToolBoxCallReturnValueW  = packed record
    Size  : integer;      // sizeof(TToolBoxCallReturnValueW)
    Name  : PWideChar;
    Value : PWideChar;
  end;
  PToolBoxCallReturnValueW      = ^TToolBoxCallReturnValueW;

  // --- array of result values ----
  TArrayToolBoxCallReturnValueW = array[0..0] of PToolBoxCallReturnValueW;
  PArrayToolBoxCallReturnValueW = ^TArrayToolBoxCallReturnValueW;

  // --- record which contains information about all return values ----
  TToolBoxCallReturnValuesW = packed record
    Size    : integer;     // sizeof(TToolBoxCallReturnValuesW)
    Count   : integer;
    Values  : PArrayToolBoxCallReturnValueW;
  end;
  PToolBoxCallReturnValuesW     = ^TToolBoxCallReturnValuesW;

  // --- exposing of return values ----
  // muss als FreeToolBoxResultsW exportiert werden!
  TFreeToolBoxResultsW          = procedure(aResults : PToolBoxCallReturnValuesW); stdcall;
